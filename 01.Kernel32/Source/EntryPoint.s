...
;;; 코드 영역 ;;;
START:
    mov ax, 0x1000  ; 보호 모드 엔트리 포인트의 시작 주소(0x10000)를 세그먼트 레지스터 값으로 변환
    mov ds, ax      ; DS 세그먼트 레지스터에 설정
    mov es, ax      ; ES 세그먼트 레지스터에 설정

    ;;; A20 게이트 활성화 ;;;
    ;;; BIOS -> 시스템 컨트롤 포트 순서로 진행 ;;;
    ; BIOS 서비스를 사용해 A20 게이트 활성화
    mov ax, 0x2401  ; A20 게이트 활성화 서비스 설정
    int 0x15        ; BIOS 인터럽트 서비스 호출

    jc .A20GATEERROR; A20 게이트 활성화 성공 확인
    jmp .A20GATESUCCESS
...
.A20GATEERROR:
    ...
    and al, 0xFE    ; 시스템 리셋 방지를 위해 0xFE(맨 오른쪽만 0)과 and연산해 비트0(맨 오른쪽)을 0으로 설정
...
.A20GATESUCCESS:
    ...
    mov cr0, eax        ; CR0 컨트롤 레지스터에 위에서 설정한 플래그를 설정하여
                        ; 보호 모드로 전환
...
PROTECTEDMODE:
    ...
    ; 화면에 보호 모드로 전환되었다는 메시지를 적는다.
    push ( SWITCHSUCCESSMESSAGE - $$ + 0x10000 )
    push 2              ; 출력할 메시지의 어드레스를 스택에 삽입
    push 0              ; 화면 Y 좌표(2)를 스택에 삽입
    call PRINTMESSAGE   ; 화면 X 좌표(0)를 스택에 삽입
...
PRINTMESSAGE:
    ...
    push esi            ; 함수에서 임시로 사용하는 레지스터를 함수의 마지막 부분에서 스택에 삽입된 값을 꺼내 원래 값으로 복원
    push edi            
    push eax
    push ecx
    push edx
...
.MESSAGELOOP:
    mov cl, byte [ esi ]        ; 메시지를 출력하는 루프
                                ; ESI 레지스터가 가리키는 문자열 위치에서 한 문자를
                                ; CL 레지스터에 복사
    ...
    jmp .MESSAGELOOP            ; 루프를 수행함

.MESSAGEEND:
    pop edx     ; 함수에서 사용이 끝난 EDX 레지스터부터 EBP 레지스터까지 스택에 삽입된 값을 이용해 복원
    pop ecx
    pop eax
    pop edi
    pop esi
...
GDT:
    ...
    ; IA-32e 모드 커널용 코드 세그먼트 디스크립터 (0x08)
    ...
    ; IA-32e 모드 커널용 데이터 세그먼트 디스크립터(0x10)
    IA_32eDATADESCRIPTOR:
        dw 0xFFFF       ; Limit [15:0]
        dw 0x0000       ; Base [15:0]
        db 0x00         ; Base [23:16]
        db 0x92         ; P=1, DPL=0, Data Segment, Read/Write
        db 0x8F         ; G=1, D=0, L=1, Limit[19:16]
        db 0x00         ; Base [31:24]

    ; 보호 모드 커널용 코드 세그먼트 디스크립터(0xq8)
    CODEDESCRIPTOR:
        dw 0xFFFF       ; Limit [15:0]
        dw 0x0000       ; Base [15:0]
        db 0x00         ; Base [23:16]
