...
// 출력 버퍼(0x60)에 수신된 데이터가 있는지 확인
BOOL kIsOutputBufferFull(void)
{
    // 상태 레지스터(0x64)에서 읽은 값에 출력 버퍼 상태 비트(비트 0)rk
    // 1로 설정되어 있으면 출력 버퍼에 키보드가 전송한 데이터 존재
...
// 입력 버퍼(0x60)에 프로세서가 쓴 데이터가 남아있는 지 확인
BOOL kIsInputBufferFull(void)
{
    // 상태 레지스터(0x64)에서 읽은 값에 입력 버퍼 상태 비트(비트 1)가
...
BOOL kActivateKeyboard(void)
{
    ...
    for(i = 0; i < 0xFFFF; i++)
    {
        // 입력 버퍼(포트0x60)가 비어있으면 키보드 커맨드 전송 가능
    ...
    // 키보드가 전달된 데이터를 최대 100개까지 수신하여 ACK 확인
    for(j = 0; j < 100; j++)
    {
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn)
{
    ...
    for(i = 0; i < 0xFFFF; i++)
    {
        // 입력 버퍼(0x60)가 비어있으면 키보드가 커맨드를 가져간 것
        if(kIsInputBufferFull() == FALSE)
            break;
    }
    ...
    for(j = 0; j < 100; j++)
    {
        for(i = 0; i < 0xFFFF; i++)
        {
            // 출력 버퍼(0x60)가 차 있으면 데이터를 읽을 수 있음
            if(kIsOutputBufferFull() == TRUE)
                break;
        }
        if(kInPortByte(0x60) == 0xFA)
    ...
    // LED 변경 값을 키보드로 전송하고 데이터가 처리될 때까지 대기
    kOutPortByte(0x60, (bCapsLockOn << 2) | (bNumLockOn << 1) | bScrollLockOn);
    for(i = 0; i < 0xFFFF; i++)
    {
        // 입력 버퍼(0x60)가 비어있으면 키보드가 LED 데이터를 가져간 것
        if(kIsInputBufferFull() == FALSE)
            break;
    }
    ...
    for(j = 0; j < 100; j++)
    {
        for(i = 0; i < 0xFFFF; i++)
        {
            ...
            // 출력 버퍼(0x60)에서 읽은 데이터가 ACK(0xFA)면 성공
            if(kIsInputBufferFull() == 0xFA)
                break;
        }
    }
    if(j >= 100)
...
/// 스캔 코드를 ASCII 코드로 변환하는 기능에 관련된 함수 ///
...
{
    // 변환 테이블에서 값을 직접 읽어서 알파벳 범위인지 확인
...
{
    // 숫자 패드가 확장 키 범위를 제외한 범위(스캔 코드 2 ~ 53)에서 영문자가 아니면
    // 숫자나 기호임
...
BOOL kIsUseCombinedCode(BYTE bScanCode)
{
    ...
    // 알파벳 키라면 Shift 키과 Caps Lock의 영향을 받음
    ...
    // 확징 키 코드가 수신되지 않았을 때만 처리 조합된 코드 사용
...
void UpdateCombinationKeyStatusAndLED( BYTE bScanCode )
{
    ...
    // 눌림 또는 떨어짐 상태 처리, 최상위 비트(비트 7)가 1이면 키가 떨어졌음을 의미
    // 0이면 눌림을 의미함
    if(bScanCode & 0x80)
    {
        bDown = FALSE;
    ...
    // Scroll Lock 키의 스캔 코드(70)이면 Scrool Lock의 상태를 갱신하고 LED 상태 변경
...
BOOL kConvertScanCodeToASCIICode(BYTE bScanCode, BYTE* pbASCIICode, BOOL* pbFlags)
{
    ...
    if(bScanCode == 0xE1)
    {
        ...
        *pbFlags = KEY_FALGS_DOWN;
        gs_stKeyboardManager.iSkipCountForPause = KEY_SKIPCOUTFORPAUSE;
        return TRUE;
    }
